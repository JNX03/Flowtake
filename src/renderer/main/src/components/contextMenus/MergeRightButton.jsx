...
import { setSelectedIds } from "../../../../src/redux/timelineSlice"
import Item from "./Item"

export default function MergeRightButton({ close, configs, selectedConfigs, onMerge, disabled = false }) {
    ...
    const isEnabled = useMemo(
        () => !disabled && selectedConfigs.length === 1 && canMergeRight(selectedConfigs[0], configs),
    ...
    const onClick = useCallback(() => {
        close()
        onMerge(mergeRight(selectedConfigs[0], configs))
    ...
    return (<Item text="Merge right" icon={ArrowRightIcon} isEnabled={isEnabled} onClick={onClick}
        kbd={<>
            <kbd className="kbd kbd-sm">m</kbd>
            <kbd className="kbd kbd-sm h-full"><ArrowLongRightIcon className="size-4" /></kbd>
        </>} />)
}

MergeRightButton.propTypes = {
