...
test("closing a project still archives the complete temp directory", async () => {
  ...
  assert.match(projects, /write_zip_directory\(src_dir, &staged_path\)/)
  assert.match(projects, /validate_project_archive\(&staged_path\)/)
  assert.match(projects, /std::fs::rename\(&staged_path, zip_path\)\?/)
  assert.match(zipDirectory, /fn zip_directory\(/)
  assert.match(zipDirectory, /std::io::copy\(&mut f, &mut zip_writer\)\?/)
  assert.match(zipDirectory, /file\.sync_all\(\)\?/)
